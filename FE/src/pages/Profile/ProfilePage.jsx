...
const ProfilePage = ({ role }) => {
  ...
  if (status.loading) {
    return (
      <main className="grid min-h-svh place-items-center bg-surface">
    ...
    <main className="min-h-svh bg-surface text-slate-950">
        ...
        <div className="mx-auto flex h-16 max-w-6xl items-center justify-between px-5 sm:px-8">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="flex h-9 items-center gap-2 rounded-full px-3 text-sm font-bold text-slate-600 transition hover:bg-red-50 hover:text-red-700"
            >
              <LogOut size={17} />{" "}
      ...
      <div className="mx-auto max-w-6xl px-5 py-8 sm:px-8 sm:py-12">
        <div className="rounded-3xl bg-gradient-to-r from-emerald-950 to-primary-deep p-6 text-white shadow-premium sm:p-8">
          <div className="flex flex-col gap-5 sm:flex-row sm:items-center">
            <div className="relative grid h-24 w-24 shrink-0 place-items-center overflow-hidden rounded-3xl bg-white/15 ring-1 ring-white/30">
              ...
              <span className="absolute bottom-1 right-1 grid h-7 w-7 place-items-center rounded-full bg-white text-primary-deep">
                <Camera size={14} />
              </span>
            </div>
        ...
        <form
          ...
          className="mt-7 grid gap-7 lg:grid-cols-[1fr_330px]"
        >
          <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-premium sm:p-7">
            ...
            <p className="mt-1 text-sm font-medium text-secondary">
              Thông tin dùng cho tài khoản và liên hệ đặt sân.
            </p>
          ...
          <aside className="space-y-7">
            <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-premium">
              <h2 className="text-lg font-extrabold">
                {isOwner ? "Thông tin sân" : "Hồ sơ thể thao"}
              </h2>
                ...
                {isOwner ? (
                  ...
                  <>
                    ...
                    <SelectField
                      ...
                      onChange={handleChange}
                      options={["Mới bắt đầu", "Trung bình", "Khá", "Nâng cao"]}
                    />
                  </>
                )}
              </div>
            ...
            {(status.message || status.error) && (
              <p
                className={`flex items-center gap-2 rounded-2xl px-4 py-3 text-sm font-bold ${status.error ? "bg-red-50 text-red-700" : "bg-emerald-50 text-emerald-800"}`}
              >
                {!status.error && <CheckCircle2 size={17} />}{" "}
