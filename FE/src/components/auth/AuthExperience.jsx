...
import { ArrowRight, CheckCircle2, LoaderCircle, Phone, ShieldCheck } from 'lucide-react';
...
const roles = [
  { value: 'Player', label: 'Người chơi', description: 'Đặt sân và kết nối bạn chơi' },
  { value: 'VenueOwner', label: 'Chủ sân', description: 'Quản lý sân và lịch đặt' },
];
...
const AuthExperience = ({ mode }) => {
  ...
  useEffect(() => {
    if (!googleReady || !import.meta.env.VITE_GOOGLE_CLIENT_ID || !googleButtonRef.current) {
      return;
    }
    ...
    renderGoogleLoginButton(googleButtonRef.current, async (credential) => {
      ...
      try {
        completeLogin(await loginWithGoogleCredential({ credential, role, fullName }));
      } catch (error) {
  ...
  const destination = useMemo(
    () => location.state?.from?.pathname || (role === 'VenueOwner' ? '/owner/profile' : '/player/profile'),
    [location.state, role]
  );
  ...
  return (
    <main className="grid min-h-svh bg-surface lg:grid-cols-[1.05fr_0.95fr]">
      <section className="relative hidden overflow-hidden bg-emerald-950 lg:block">
        <img src="/assets/login-athlete.png" alt="Vận động viên SportsHub" className="absolute inset-0 h-full w-full object-cover" />
        ...
        <div className="relative flex min-h-svh flex-col justify-between p-12 text-white xl:p-16">
          ...
          <div className="max-w-xl">
            <p className="text-sm font-extrabold uppercase tracking-[0.22em] text-emerald-200">Một tài khoản, mọi trận đấu</p>
            <h1 className="mt-5 text-5xl font-extrabold leading-tight">Đặt sân nhanh. Chơi hết mình.</h1>
            <p className="mt-5 max-w-lg text-base font-medium leading-8 text-emerald-50/90">
              Đăng nhập an toàn bằng Google hoặc mã OTP gửi qua Zalo. Hồ sơ của bạn luôn sẵn sàng trên mọi thiết bị.
            </p>
          </div>
          <div className="flex gap-6 text-sm font-bold text-emerald-100">
            <span className="flex items-center gap-2"><ShieldCheck size={18} /> Bảo mật phiên</span>
            <span className="flex items-center gap-2"><CheckCircle2 size={18} /> Xác thực nhanh</span>
          </div>
        </div>
      </section>
      ...
      <section className="flex min-h-svh items-center justify-center px-5 py-10 sm:px-8">
        ...
        >
          ...
          <p className="text-sm font-extrabold text-primary-deep">{isRegister ? 'Bắt đầu với SportsHub' : 'Chào mừng trở lại'}</p>
          ...
          <p className="mt-3 text-sm font-medium leading-6 text-secondary">
            {isRegister ? 'Chọn loại tài khoản và phương thức xác thực phù hợp.' : 'Tiếp tục hành trình thể thao của bạn.'}
          </p>
          ...
          <div className="mt-7 grid grid-cols-2 gap-3">
            {roles.map((item) => (
              ...
              >
                ...
                <span className="mt-1 block text-xs font-medium leading-5 text-secondary">{item.description}</span>
              </button>
            ))}
          </div>
          ...
          {isRegister && (
            <label className="mt-5 block">
              <span className="mb-2 block text-sm font-extrabold text-slate-700">Họ và tên</span>
              <input
                value={fullName}
          ...
          {import.meta.env.VITE_GOOGLE_CLIENT_ID ? (
            ...
            >
              <span className="grid h-7 w-7 place-items-center rounded-full border border-slate-200 font-extrabold text-blue-600">G</span>
              Tiếp tục với Google
            </button>
          )}

          <div className="my-6 flex items-center gap-3 text-xs font-bold text-slate-400">
            <span className="h-px flex-1 bg-slate-200" /> HOẶC OTP ZALO <span className="h-px flex-1 bg-slate-200" />
          </div>

          <form onSubmit={handleOtp} className="space-y-4">
            <label className="block">
              <span className="mb-2 block text-sm font-extrabold text-slate-700">Số điện thoại Zalo</span>
            ...
            {otpSent && (
              <label className="block">
                <span className="mb-2 flex items-center justify-between text-sm font-extrabold text-slate-700">
                  Mã OTP 6 số
                  <button type="button" onClick={() => { setOtpSent(false); setOtp(''); }} className="text-xs text-primary-deep">
                    Đổi số điện thoại
                  </button>
                </span>
                <input
            ...
            {status.error && <p className="rounded-xl bg-red-50 px-4 py-3 text-sm font-bold text-red-700">{status.error}</p>}
            {status.message && (
              <p className="rounded-xl bg-emerald-50 px-4 py-3 text-sm font-bold text-emerald-800">
                {status.message}{devOtp && <> Mã dev: <strong>{devOtp}</strong></>}
              </p>
            )}
            ...
            >
              {status.loading ? <LoaderCircle className="h-5 w-5 animate-spin" /> : otpSent ? 'Xác nhận và tiếp tục' : 'Gửi mã OTP qua Zalo'}
          ...
          <p className="mt-7 text-center text-sm font-medium text-secondary">
            ...
            <Link to={isRegister ? '/login' : '/register'} className="font-extrabold text-primary-deep">
              {isRegister ? 'Đăng nhập' : 'Đăng ký ngay'}
            </Link>
          </p>
          {!import.meta.env.VITE_GOOGLE_CLIENT_ID && (
            <p className="mt-4 text-center text-xs font-medium text-slate-400">Google đang chạy ở chế độ demo do chưa cấu hình Client ID.</p>
          )}
        </motion.div>
      </section>
    </main>
